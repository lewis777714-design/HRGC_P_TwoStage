...
some metric, that is reported faithfully.
    ...
    B6  DistilBERT-base CLS, single-stage flat
    B7  RoBERTa-base CLS, single-stage flat
    B8  Sentence-BERT cosine similarity (zero-shot)
...
Run:
    python baselines.py --data <csv>           # all 8 baselines
...
ONLY       = None          # None = all baselines. Or e.g. "B1,B3,B8".
from model import (
    AttentionPooling,
...
def _simple_param_groups(model, bert_lr, head_lr, weight_decay,
    ...
    cross-backbone baselines (B6 DistilBERT, B7 RoBERTa) so that
    layer-naming differences don't give some baselines an unfair
...
# ─────────────────────────────────────────────────────────────────
# B6 / B7 — Different encoder backbones (CLS-pool flat heads)
# ─────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────
# B8 — Sentence-BERT label-name cosine similarity (zero-shot)
# ─────────────────────────────────────────────────────────────────
...
def run_B8_sbert_zero_shot(splits, mappings, *, tokenizer, cfg, device,
                           save_dir, labels_full):
...
BASELINES = {
    ...
    "B8": ("[B8] Sentence-BERT (mpnet-base-v2) zero-shot label-name cosine",
           run_B8_sbert_zero_shot, True),
}
...
def main():
    ...
    p.add_argument("--only",       type=str, default=None,
                   help="Comma-separated baselines, e.g. B1,B3,B8.")
    ...
    for name in selected:
        ...
        # Sentinel file tracks native-crash retries (e.g. CUDA ACCESS_VIOLATION
        # exit-code -1073741819 bypasses the finally block so the sentinel
        # persists across restarts).  Allow up to MAX_CRASH_RETRIES attempts
        ...
        if os.path.exists(sentinel_path):
            try:
                with open(sentinel_path, "r", encoding="utf-8") as f:
                    crash_count = json.load(f).get("crash_count", 0) + 1
            except Exception:
                crash_count = 1
            if crash_count >= MAX_CRASH_RETRIES:
                print(f"[skip] {name}: crashed {crash_count} time(s), giving up")
                results[name] = {"description": desc,
                                 "error": f"process crashed {crash_count} time(s)"}
                with open(os.path.join(root_dir, "baselines_results.json"),
                          "w", encoding="utf-8") as f:
                    json.dump(results, f, indent=2, default=str)
                continue
            print(f"[retry] {name}: previous crash detected "
                  f"(crash #{crash_count}/{MAX_CRASH_RETRIES}), retrying...")
